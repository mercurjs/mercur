...
export class Migration20250522062207 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`drop table if exists "seller_verification" cascade;`);

    this.addSql(`alter table if exists "seller" add column if not exists "verification_status" text check ("verification_status" in ('pending', 'in_progress', 'verified', 'rejected')) not null default 'pending', add column if not exists "status" text check ("status" in ('active', 'inactive')) not null default 'inactive';`);
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`alter table if exists "seller" drop column if exists "verification_status", drop column if exists "status";`);
  }

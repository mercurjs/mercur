...
export class Migration20241216112612 extends Migration {

  async up(): Promise<void> {
    this.addSql('create table if not exists "payout" ("id" text not null, "account_reference_id" text not null, "currency_code" text not null, "amount" numeric not null, "data" jsonb null, "raw_amount" jsonb not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "payout_pkey" primary key ("id"));');
    this.addSql('CREATE INDEX IF NOT EXISTS "IDX_payout_deleted_at" ON "payout" (deleted_at) WHERE deleted_at IS NULL;');

    this.addSql('create table if not exists "payout_account" ("id" text not null, "status" text check ("status" in (\'pending\', \'active\', \'disabled\')) not null default \'pending\', "reference_id" text null, "data" jsonb null, "context" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "payout_account_pkey" primary key ("id"));');

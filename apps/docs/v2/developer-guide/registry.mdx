...
A registry is a set of JSON files — one per block — served over HTTP. When you run `npx @mercurjs/cli@canary add`, the CLI fetches the block's JSON, resolves its dependencies, and copies the source files into your project.
...
| ---------------------- | -------------------------------------------------------------- |
| `name`                 | Block identifier used in `npx @mercurjs/cli@canary add`               |
...
```bash
npx @mercurjs/cli@canary add @my-org/reviews
```

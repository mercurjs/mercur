...
The Mercur CLI (`@mercurjs/cli@canary`) manages your marketplace project — creating projects, installing blocks, generating types, and checking for updates.
...
```bash
bunx @mercurjs/cli@canary create my-marketplace
```
...
```bash
bunx @mercurjs/cli@canary add wishlist
bunx @mercurjs/cli@canary add product-review order-tracking
```
...
```bash
bunx @mercurjs/cli@canary search --query wishlist
```
...
```bash
bunx @mercurjs/cli@canary view wishlist
```
...
```bash
bunx @mercurjs/cli@canary diff wishlist
```

If there are changes you want, update with:

```bash
bunx @mercurjs/cli@canary add wishlist --overwrite
```
...
```bash
bunx @mercurjs/cli@canary codegen
```
...
```bash
bunx @mercurjs/cli@canary build
```
...
```bash
bunx @mercurjs/cli@canary telemetry --disable
bunx @mercurjs/cli@canary telemetry --enable
```
...
```bash
bunx @mercurjs/cli@canary search --query review --registry @my-registry
```

...
The Mercur CLI (`@mercurjs/cli`) manages your marketplace project — creating projects, installing blocks, generating types, and checking for updates.
...
```bash
npx @mercurjs/cli add wishlist
npx @mercurjs/cli add product-review order-tracking
```
...
```bash
npx @mercurjs/cli search --query wishlist
```
...
```bash
npx @mercurjs/cli view wishlist
```
...
```bash
npx @mercurjs/cli diff wishlist
```

If there are changes you want, update with:

```bash
npx @mercurjs/cli add wishlist --overwrite
```
...
```bash
npx @mercurjs/cli telemetry --disable
npx @mercurjs/cli telemetry --enable
```
...
```bash
npx @mercurjs/cli search --query review --registry @my-registry
```

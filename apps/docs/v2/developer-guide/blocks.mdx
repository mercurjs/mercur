...
```bash
bunx @mercurjs/cli@canary add reviews
bunx @mercurjs/cli@canary add wishlist product-import-export
```
...
4. **Regenerate types** with `bunx @mercurjs/cli@canary codegen`
...
```bash
bunx @mercurjs/cli@canary diff reviews
```

If there are updates you want, re-install with the overwrite flag:

```bash
bunx @mercurjs/cli@canary add reviews --overwrite
```
...
```bash
bunx @mercurjs/cli@canary search
...
```bash
bunx @mercurjs/cli@canary view reviews
```

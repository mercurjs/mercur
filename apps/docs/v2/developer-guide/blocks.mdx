...
```bash
npx @mercurjs/cli add reviews
npx @mercurjs/cli add wishlist product-import-export
```
...
4. **Regenerate types** with `npx @mercurjs/cli codegen`
...
```bash
npx @mercurjs/cli diff reviews
```

If there are updates you want, re-install with the overwrite flag:

```bash
npx @mercurjs/cli add reviews --overwrite
```

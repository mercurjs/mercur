...
Core-admin/vendor panels ship 34+ standard pages. If you only used standard Mercur pages, you don't need to port any dashboard code.
...
| `@hooks/*`, `@components/*`, `@lib/*` | `@mercurjs/dashboard-shared` |

Move pages from `src/routes/` to `src/pages/` with `export default`.

Most 1.x custom hooks become unnecessary — `@mercurjs/client` generates a typed client from route codegen.

...
By default, new sellers start as `PENDING` and require manual admin approval. You can configure the seller module to automatically approve sellers on creation by passing the `autoApprove` option:
...
When `autoApprove` is `true`, new sellers are created with `ACTIVE` status immediately, skipping the manual approval step. If an explicit status is provided during creation, it takes precedence over this option.
...
### Register a seller (Vendor API)

The seller registration endpoint allows unauthenticated users to create a new seller account. The seller starts in `PENDING_APPROVAL` status.
...
### Create a seller (Admin API)

Admins can create sellers directly with a specified status and send a member invite:
...
### Approve a seller (Admin API)

Approves a seller in `PENDING_APPROVAL` status, changing it to `OPEN`:

```bash
curl -X POST http://localhost:9000/admin/sellers/slr_abc123/approve \
  -H "Authorization: Bearer <admin-token>"
```

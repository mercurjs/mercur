...
The [API Client](/v2/developer-guide/api-client) (`@mercurjs/client`) provides type-safe access to all Mercur API endpoints. Types are generated from your actual API routes using `npx @mercurjs/cli@canary codegen`, so requests and responses are always in sync with your backend.

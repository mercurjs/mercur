...
The [Mercur CLI](/v2/developer-guide/cli) (`@mercurjs/cli@canary`) manages your project — scaffolding from templates, installing blocks, searching registries, generating types, and diffing local changes against upstream versions.
...
The [API Client](/v2/developer-guide/api-client) (`@mercurjs/client`) provides type-safe access to all Mercur API endpoints. Types are generated from your actual API routes using `bunx @mercurjs/cli@canary codegen`, so requests and responses are always in sync with your backend.

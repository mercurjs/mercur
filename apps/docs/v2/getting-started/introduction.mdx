...
- **Adding features via CLI** — An AI agent can run `mercurjs add reviews` to install a complete review system — data models, API routes, workflows, and UI — in one command. No guessing at file structure or imports. The CLI handles wiring, just like Astro's `astro add` command.
...
- **Generating typed integrations** — AI reads the generated route types from `mercurjs codegen` and produces API calls that are correct by construction. No hallucinated endpoints, no wrong payloads — the types are the source of truth.
- **Building UI pages** — AI can scaffold an admin or vendor page by creating a file in `src/pages/`. The dashboard SDK picks it up automatically through file-based routing. No manual route registration, no config files to update.
- **Modifying blocks it can diff** — AI can compare local code against the registry with `mercurjs diff`, understand what changed, and make targeted modifications instead of blind overwrites.

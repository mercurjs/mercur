...
<Info>
  **Choose the right tool.** A component override replaces one of four global layout slots across the whole panel — the heaviest tool, for chrome that isn't tied to a page. To *add* to a page use a [widget](/rc/resources/tutorials/add-a-widget) or [custom field](/rc/resources/tutorials/extend-forms-and-tables); to restructure one page [re-compose it](/rc/resources/tutorials/recompose-a-page); to add a new screen [drop in a route](/rc/resources/tutorials/custom-panel-page). The decision guide in [Extending Panels](/rc/resources/customization/extending-panels#choosing-your-extension-mechanism) compares them all.
</Info>
...
<AccordionGroup>
  ...
  <Accordion title="Can I override other components, like the login page or a form?">
    These four slots are the complete list of *whole-component* overrides. But you don't need an override for most things: the login screen has [widget zones](/rc/resources/tutorials/add-a-widget) (`login.logo.*`, `login.before.*`, `login.after.*`), forms are extended with [custom fields](/rc/resources/tutorials/extend-forms-and-tables), and page-level structure is changed through drop-in routes and [compound re-composition](/rc/resources/tutorials/recompose-a-page).
  </Accordion>

...
<Steps>
  ...
  <Step title="Understand the zone id">
    ...
    | `replace` | Replaces the built-in content of the zone |

    Multiple `before` / `after` widgets on the same zone stack in registration order. A single `replace` widget swaps the built-in content.
  </Step>

...
description: "Add pages, inject widgets, customize navigation, extend forms and tables, and replace components in the admin and vendor panels."
---

Both the admin panel and vendor portal use the same SDK (`@mercurjs/dashboard-sdk`). Customization is **file-based and convention-driven** — you add pages, widgets, and field extensions by dropping files under `src/`, configure navigation through a single file, and override global chrome through the plugin config. Nothing is registered by hand; the SDK crawls your `src/` at build time and wires everything in.

<Info>
  **Coming from Medusa?** The extension model is deliberately Medusa-shaped. The helpers you know — `defineWidgetConfig`, `defineRouteConfig`, `defineCustomFieldsConfig`, `createFormHelper` — all exist here, re-exported from `@mercurjs/dashboard-sdk` (without Medusa's `unstable_` prefix). What's different: Mercur ships **two** panels (admin *and* vendor) from one framework, so a file only ever targets the panel it lives in (there is no `surface` field), and widget zones carry a `before | after | replace` placement suffix on the zone id. Everything is additive by default — your contribution augments the built-in page instead of replacing it.
</Info>

## The extension mechanisms

Mercur gives you two complementary families of tools:

**Additive extensions** — augment a built-in page without owning it. One concern per file, discovered by the build-time crawl:
...
**Full-composition escape hatches** — take ownership when additive isn't enough:

- **Drop-in routes** — add a brand-new page, or replace a built-in page at the same path.
- **Compound re-composition** — render a built-in page and swap only the slots you care about.
- **Component overrides** — replace one of four global layout slots (sidebar, topbar, store setup).

<Note>
  Prefer the additive tools. A widget, a nav override, or a custom field leaves the rest of the built-in page — data fetching, filters, pagination, i18n — completely intact. Reach for a full-page replacement or a component override only when you genuinely need to own the whole thing.
</Note>

## Choosing your extension mechanism

Rule of thumb: **injecting UI into an existing page? Use a widget. Adding data to a form or section? Use a custom field. Reshaping the sidebar? Use the navigation file. Adding a whole new screen? Drop in a route. Rewiring an existing page's structure? Re-compose it. Changing the frame around every page? Override a component.**
...
| Change the structure of an existing page | A drop-in route **at the same path**, re-composing the page's slots | Untouched slots keep their behavior — see [Re-compose a page](/rc/resources/tutorials/recompose-a-page) |
| Change global chrome (sidebar, topbar, store setup) | A component override in the plugin config | One component, applied everywhere its slot renders — see [Replace components](/rc/resources/tutorials/replace-panel-components) |
...
- `replace` — swap the built-in content of that zone entirely.

Zones mounted today (**vendor portal**):

| Zone id | Where it renders |
|---------|------------------|
| `product.list.before` / `.after` / `.replace` | Vendor product list page |
...
<Info>
  **Matching paths replace, new paths append.** If your route's path matches a built-in page (e.g. `src/routes/products/page.tsx` → `/products`), your page **replaces** the built-in one — that's the full-replacement override mechanism. Any other path is added alongside the built-in routes. Delete the file and the built-in page returns. To change *part* of a built-in page without owning it, prefer a widget or a custom field, or [re-compose the page](/rc/resources/tutorials/recompose-a-page).
</Info>
...
For global chrome that isn't tied to one page, the SDK exposes four named component overrides. Point the plugin config at your own file and the panel renders it in place of the built-in one everywhere it appears:
...
<Warning>
  These four slots are the complete list — there is no generic "replace any component" registry. If what you want to change is a single page, [re-compose it](/rc/resources/tutorials/recompose-a-page); if it's a spot inside a page, use a [widget](#widgets) or a [custom field](#custom-fields).
</Warning>

The [Replace components](/rc/resources/tutorials/replace-panel-components) tutorial walks through it in full.
...
<AccordionGroup>
  <Accordion title="Can I use Medusa's defineWidgetConfig and widget zones?">
    Yes. `defineWidgetConfig` is re-exported from `@mercurjs/dashboard-sdk` and drives file-based widgets under `src/widgets/`. The difference from Medusa is the `before | after | replace` placement suffix on the zone id and that each panel ships its own typed zone set. See [Add a widget](/rc/resources/tutorials/add-a-widget).
  </Accordion>
  ...
  <Accordion title="How do I change just one part of a built-in page?">
    For a spot inside the page (a banner, an extra field, a column), use a [widget](#widgets) or a [custom field](#custom-fields) — the rest of the page keeps all its behavior. To restructure the page itself, import it from the `/pages` subpath (`@mercurjs/vendor/pages` or `@mercurjs/admin/pages`), drop a route at the same path, and [re-compose its slots](/rc/resources/tutorials/recompose-a-page).
  </Accordion>

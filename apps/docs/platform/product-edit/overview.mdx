...
Use Mercur to govern how products change on your marketplace.

Products live in a single shared catalog, so an edit can't just be written to a
product row. It has to be captured, reviewed, and applied deliberately. The
Product Edit domain is that pipeline. Every edit becomes an immutable
`ProductChange` record with typed actions, a status lifecycle, and a full audit
trail of who changed what and who approved it.
...
- **Immutable change records:** every edit is a `ProductChange` that is never rewritten, only appended to and resolved.
...
- **Governed lifecycle:** a change moves `pending` → `confirmed` / `declined` / `canceled`, each through a dedicated workflow.

...
Use Mercur to run a single shared product catalog that many stores sell against.

The Catalog domain is the marketplace's product surface. It holds master
products, variants, categories, collections, tags, and types, plus the
marketplace layer that controls which stores may sell which products and how new
submissions are reviewed. A store never owns a product. It sells against a shared
master product through an [offer](/platform/offer/overview).
...
- **Shared master catalog:** products live in one catalog, not owned by any store. Creating a product adds it to the shared catalog.
- **Store allowlist:** the `product_seller` link controls which stores may sell a given master product.
- **Governed submissions:** a `draft` → `proposed` → `published` / `rejected` lifecycle, with vendor-created products defaulting to `proposed`.

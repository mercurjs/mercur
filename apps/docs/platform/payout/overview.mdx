...
Use Mercur to onboard sellers to a payment provider, capture authorized
payments, and transfer earnings to each seller after fulfillment.

The Payout domain is the money-out side of the marketplace. Once an order is
placed and fulfilled, it settles the seller's share to their connected account.
The seller's share is the order total minus commission. The domain ships with a
pluggable provider interface and a **Stripe Connect** implementation out of the
box. It also runs a fully automated capture-and-transfer pipeline driven by
scheduled jobs and provider webhooks.

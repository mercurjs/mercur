...
<Note>
  A Mercur module is a standard [Medusa module](https://docs.medusajs.com/learn/fundamentals/modules). The examples below build a small **Brand** module. It is the kind of custom module you'd add to your own project alongside the built-in ones, so the rules stand on their own rather than relying on Mercur internals.
</Note>
...
Only add a custom method when the logic is **about this module's own data** and can't be expressed with the generated methods, such as a specialised query. When you do, use Medusa's DI decorators so the method runs in the ambient context:

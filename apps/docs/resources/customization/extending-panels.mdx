...
<AccordionGroup>
  ...
  <Accordion title="Why does my widget / custom field / nav override not type-check?">
    Make sure the panel's typed targets are registered with a single `src/extension-targets.d.ts` containing `/// <reference types="@mercurjs/vendor/extension-targets" />` (or the admin equivalent). Without it, zone/model/nav ids aren't known to TypeScript. `create-mercur-app` ships this file.
  </Accordion>

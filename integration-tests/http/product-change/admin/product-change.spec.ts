...
/**
 * Integration coverage for the SPEC-008 step 4 product-change workflow
 * group. Runs against a Medusa container with the new
 * `product-attribute` + `product-change` modules registered AND the
 * legacy fused `@mercurjs/core/modules/product` module retired.
 *
 * **Gated on SPEC-008 step 5 (the `withMercur()` module-registration
 * swap).** Until that ships, the new modules collide with the legacy
 * fused module on entity names (`ProductChange`, `ProductAttribute`,
 * shared `prodch_`/`pattr_` id prefixes, identical table names) so
 * the container cannot boot with both side-by-side. The suite is
 * enabled with `SPEC_008_STEP_5_LANDED=true bun run test:integration:http
 * -- product-change`.
 *
...
if (STEP_5_LANDED) {
  ...
  describe.skip("Admin - Product change lifecycle (SPEC-008 step 5 gated)", () => {
    it.skip(

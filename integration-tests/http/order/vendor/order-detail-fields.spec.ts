...
// Mirrors the field set the vendor panel order-detail page requests
// (packages/vendor/src/pages/orders/[id]/constants.ts). The SPEC-011
// commission redesign removed the order -> commission link, so requesting
// `commission_value` / `items.commission_lines` here used to crash the
// order-module populate with "Cannot read properties of undefined (reading
// 'strategy')". This guards against that field list regressing again.
const VENDOR_ORDER_DETAIL_FIELDS = [
    "id",
    "status",
    "display_id",
    "currency_code",
...
medusaIntegrationTestRunner({
    testSuite: ({ getContainer, api }) => {
        describe("Vendor - Order detail field set", () => {
            ...
            beforeEach(async () => {
                ...
                sellerSeed = await seedSellerOfferWithShipping({
                    email: "detail-seller@test.com",
                    name: "DetailS1",
                })
            })

            it("returns the order with the full vendor panel field set", async () => {
                const order = await completeCartCheckout(sellerSeed.offer.id)

                const response = await api.get(
                    `/vendor/orders/${order.id}?fields=${encodeURIComponent(
                        VENDOR_ORDER_DETAIL_FIELDS
                    )}`,
                    sellerSeed.headers
                )

...
medusaIntegrationTestRunner({
    testSuite: ({ getContainer, api }) => {
        describe("Vendor - Payment Collection Mark As Paid", () => {
            ...
            const getOrderPaymentCollection = async (
                orderId: string,
                headers: any
            ) => {
                const response = await api.get(
                    `/vendor/orders/${orderId}?fields=*payment_collections`,
                    headers
                )
                return response.data.order.payment_collections[0]
            }
            ...
            describe("POST /vendor/payment-collections/:id/mark-as-paid", () => {
                it("rejects a request from a seller that does not own the order", async () => {
                    ...
                    const response = await api
                        .post(
                            `/vendor/payment-collections/${paymentCollection.id}/mark-as-paid`,
                            { order_id: order.id },
                            seller2Seed.headers
                        )
                        .catch((e) => e.response)
                ...
                it("rejects when the payment collection does not exist", async () => {
                    const response = await api
                        .post(
                            `/vendor/payment-collections/paycol_does_not_exist/mark-as-paid`,

...
/**
 * Integration coverage for the SPEC-008 step 4 sub-step A
 * product-attribute workflow group. Exercises the CRUD + upsert +
 * batch surface that lives at
 * `packages/core/src/workflows/product-attribute/workflows/*`.
 *
 * **Gated on SPEC-008 step 5** — see sibling `product-change.spec.ts`
 * for the rationale (the new `product-attribute` module is registered
 * by the step 5 swap in `withMercur()`).
 */
...
if (STEP_5_LANDED) {
  ...
  describe.skip(
    "product-attribute workflow group (SPEC-008 step 5 gated)",
    () => {
      it.skip(

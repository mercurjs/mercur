...
export const PossibleValuesTable = ({ attribute }: PossibleValuesTableProps) => {
            ...
            <DropdownMenu>
              <DropdownMenu.Trigger asChild>
                <Button
                  variant="transparent"
                  size="small"
                  data-testid={`attribute-possible-value-action-menu-trigger-${possibleValue.id}`}
                >
                  <EllipsisHorizontal />
                </Button>
              ...
              <DropdownMenu.Content
                align="end"
                data-testid={`attribute-possible-value-action-menu-${possibleValue.id}`}
              >
                <DropdownMenu.Item
                  onClick={() => {
                    if (attributeId) {
                      navigate(
                        `possible-values?possible_value_id=${possibleValue.id}`
                      )
                    } else {
  ...
  return (
    <Container
      ...
      data-testid="attribute-possible-values-table-container"
    >
      <div
        className="flex items-center justify-between px-6 py-4"
        data-testid="attribute-possible-values-table-header"
      >
        <Heading
          level="h2"
          data-testid="attribute-possible-values-table-heading"
        >
          Possible Values
        </Heading>
      </div>
      <div data-testid="attribute-possible-values-table-wrapper">
        <DataTable
          instance={possibleValuesTable}
          data-testid="attribute-possible-values-table"
        >
          <DataTable.Toolbar
            className="flex flex-col items-start justify-between gap-2 md:flex-row md:items-center"
            data-testid="attribute-possible-values-table-toolbar"
          >
            <DataTable.Search
              placeholder="Search possible values..."
              data-testid="attribute-possible-values-table-search"
            />
          </DataTable.Toolbar>

...
export const EditApiKeyForm = ({ apiKey }: EditApiKeyFormProps) => {
  ...
  return (
    <RouteDrawer.Form
      form={form}
      data-testid={`${apiKey.type}-api-key-edit-form`}
    >
          ...
          <div className="flex flex-col gap-y-4">
            <Form.Field
              ...
              render={({ field }) => {
                return (
                  <Form.Item
                    data-testid={`${apiKey.type}-api-key-edit-title-item`}
                  >
                    <Form.Label
                      data-testid={`${apiKey.type}-api-key-edit-title-label`}
                    >
                      {t("fields.title")}
                    </Form.Label>
                    <Form.Control
                      data-testid={`${apiKey.type}-api-key-edit-title-control`}
                    >
                      <Input
                        {...field}
                        data-testid={`${apiKey.type}-api-key-edit-title-input`}
                      />
                    </Form.Control>
                    <Form.ErrorMessage
                      data-testid={`${apiKey.type}-api-key-edit-title-error`}
                    />
                  </Form.Item>
                )
              }}
          ...
          <div className="flex items-center gap-x-2">
            <RouteDrawer.Close asChild>
              <Button
                size="small"
                variant="secondary"
                data-testid={`${apiKey.type}-api-key-edit-cancel-button`}
              >
                {t("actions.cancel")}
              </Button>
            </RouteDrawer.Close>
            <Button
              ...
              type="submit"
              isLoading={isPending}
              data-testid={`${apiKey.type}-api-key-edit-save-button`}
            >
              {t("actions.save")}
            </Button>
          </div>

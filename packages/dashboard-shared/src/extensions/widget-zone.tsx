...
export type WidgetZoneProps = {
  ...
  data?: unknown
  /** The built-in content, rendered between `before` and `after` widgets and
   * swapped out entirely when a `replace` widget is registered. */
  children?: ReactNode
}

/**
 * Injection-zone host. Renders `before` widgets → the built-in child (or the
 * single `replace` widget) → `after` widgets. A zone that no page renders as a
 * host can never be targeted.
 */
export const WidgetZone = ({ id, data, children }: WidgetZoneProps) => {
  const { before, replace, after } = useExtension().getWidgets(id)
  ...
  return (
    <Fragment>
      ...
      {replace ? (
        <replace.Component key={`replace-${replace.widgetId}`} data={data} />
      ) : (
        children
      )}

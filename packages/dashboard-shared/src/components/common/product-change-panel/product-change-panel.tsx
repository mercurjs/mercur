...
const ImageStrip = ({
  ...
  <div className={clx("flex flex-wrap items-center gap-1.5", faded && "opacity-50")}>
    ...
    return (
      <div className="flex flex-wrap items-center gap-2">
        ...
        {isImageList(diff.next) ? (
          ...
          <ValueText>{formatFieldValue(diff.next, diff.field)}</ValueText>
        )}
      </div>
    )
  }
  ...
  if (isReferenceField(diff.field)) {
    return (
      <ValueText>
        ...
        {hasPrev && " → "}
        <ReferenceValue field={diff.field} value={diff.next} lookups={lookups} />
      </ValueText>
    )
  }
...
const attributeDisplay = (
  ...
  const entry = change.attributeId
    ? lookup.get(change.attributeId)
    : undefined
  ...
  if (change.kind === "added") {
    ...
    if (change.scalarValue !== undefined && change.scalarValue !== null) {
      names.push(String(change.scalarValue))
    }
    return { name, previous: null, next: names.length ? names.join(", ") : "-" }
  }
  ...
  if (change.scalarValue !== undefined && change.scalarValue !== null) {
    ...
    return {
      name,
      previous: prev ?? null,
      next: String(change.scalarValue),
    }
  }
...
export const ProductChangePanel = ({
  ...
  const attributeIdsToFetch = useMemo(() => {
    ...
    for (const change of view.attributes) {
      if (change.attributeId && !productAttributesById.has(change.attributeId)) {
  ...
  return (
    <Container className="divide-y p-0" data-testid="product-active-edit-section">
      ...
      {headerDescription && <div className="px-6 py-4">{headerDescription}</div>}
          ...
          {hasUpdatedGroup && (
            ...
            >
              {view.productUpdated.map((diff, idx) => (
                <ItemRow key={`u-${diff.field}-${idx}`} label={fieldLabel(diff.field)}>
    ...
    <>
      ...
      >
        ...
        <div className="flex-1">
          <VariantIdentity variant={variant} variantFallback={variantFallback} />
        </div>
      </div>
      ...
      {group.fieldDiffs.length > 0 && (
        <GroupRow label={updatedLabel}>
          {group.fieldDiffs.map((diff, idx) => (
            <ItemRow key={`vu-${diff.field}-${idx}`} label={fieldLabel(diff.field)}>
              <FieldDiffValue diff={diff} lookups={lookups} />
            </ItemRow>
          ))}
        </GroupRow>

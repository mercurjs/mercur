...
const inventoryDetailQuery = (id: string) => ({
  ...
  queryFn: async () =>
    fetchQuery(`/vendor/inventory-items/${id}?fields=${INVENTORY_DETAIL_FIELDS}`, { method: "GET" }),
})
...
export const Breadcrumb = (props: InventoryDetailBreadcrumbProps) => {
  ...
  const { inventory_item } = useInventoryItem(id!, { fields: INVENTORY_DETAIL_FIELDS }, { initialData: props.data, enabled: Boolean(id) })
  ...
  return (
    ...
    >
      ...
      <TwoColumnPage.Sidebar>
        <InventoryItemVariantsSection variants={(inventory_item as any).variants} />

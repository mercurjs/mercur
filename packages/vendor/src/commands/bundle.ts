...
export async function bundle() {
  const vendorDir = path.resolve(process.cwd(), "src", "vendor");

  const pathExists = await fse.pathExists(vendorDir);

  if (!pathExists) {
    logger.panic(
      "The `src/vendor` directory could not be found. It appears that your project does not contain any admin extensions."
    );
  }
  ...
  const [routes, widgets, settings] = await Promise.all([
    findAllValidRoutes(path.resolve(vendorDir, "routes")),
    findAllValidWidgets(path.resolve(vendorDir, "widgets")),
    findAllValidSettings(path.resolve(vendorDir, "settings")),
  ]);
  ...
  const dist = path.resolve(process.cwd(), "dist", "vendor");
  ...
  try {
    ...
    await bundle.write({
      dir: path.resolve(process.cwd(), "dist", "vendor"),

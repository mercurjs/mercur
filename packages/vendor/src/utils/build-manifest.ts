...
async function getLastTimeModifiedAt(appDir: string) {
  const vendorPath = path.resolve(appDir, "src", "vendor");
  ...
  const pathExists = await fse.pathExists(vendorPath);
  ...
  await processFolder(vendorPath);

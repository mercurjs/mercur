...
export default function (_rootDirectory: string, options: PluginOptions) {
  ...
  if (isDevelop) {
    app.get(`${path}`, (_req, res) => {
      res.send(
        "Vendor is running in development mode. See the terminal for the URL."
      );
    });
  ...
  if (serve) {
    const buildPath: string = resolve(process.cwd(), outDir || "build-vendor");

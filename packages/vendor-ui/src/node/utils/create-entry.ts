...
async function createLocalExtensionsEntry(appDir: string, dest: string) {
  const localAdminDir = path.resolve(appDir, "src", "vendor");
  ...
  const copied = await copyLocalExtensions(
    localAdminDir,
    path.resolve(dest, "vendor", "src", "extensions")
  );
  ...
  const [localWidgets, localRoutes, localSettings] = await Promise.all([
    findAllValidWidgets(
      path.resolve(dest, "vendor", "src", "extensions", "widgets")
    ),
    findAllValidRoutes(
      path.resolve(dest, "vendor", "src", "extensions", "routes")
    ),
    findAllValidSettings(
      path.resolve(dest, "vendor", "src", "extensions", "settings")
    ),
  ]);
  ...
  const widgetsArray = localWidgets.map((file, index) => {
    const relativePath = normalizePath(
      path
        .relative(path.resolve(dest, "vendor", "src", "extensions"), file)
  ...
  const routesArray = localRoutes.map((route, index) => {
    const relativePath = normalizePath(
      path
        .relative(path.resolve(dest, "vendor", "src", "extensions"), route.file)
        .replace(FILE_EXT_REGEX, "")
    );
  ...
  try {
    await fse.outputFile(
      path.resolve(dest, "vendor", "src", "extensions", "_local-entry.ts"),
      extensionsEntry
    );
  } catch (err) {
    logger.panic(
...
async function writeTailwindContentFile(dest: string, plugins: string[]) {
  const tailwindContent = dedent`
    ...
    const devPath = path.join(__dirname, "..", "..", "src/vendor/**/*.{js,jsx,ts,tsx}")
        ...
        ${plugins
          .map((plugin) => {
            const tailwindContentPath = normalizePath(
              path.relative(
                path.resolve(dest, "vendor"),
                path.dirname(path.join(plugin, "..", ".."))
              )
            );

            return `"${tailwindContentPath}/dist/vendor/**/*.{js,jsx,ts,tsx}"`;
          })
          .join(",\n")}
      ],
    }
  ...
  try {
    await fse.outputFile(
      path.resolve(dest, "vendor", "tailwind.content.js"),
      tailwindContent
    );
  } catch (err) {
    logger.warn(
      `Failed to write the Tailwind content file to ${dest}. The vendor UI will remain functional, but CSS classes applied to extensions from plugins might not have the correct styles`
    );
  }
}
...
async function createMainExtensionsEntry(
  ...
  if (!plugins.length && !hasLocalExtensions) {
    ...
    try {
      await fse.outputFile(
        path.resolve(dest, "vendor", "src", "extensions", "_main-entry.ts"),
        emptyEntry
      );
    } catch (err) {
      logger.panic(
  ...
  const pluginsArray = plugins.map((plugin) => {
    const relativePath = normalizePath(
      path
        .relative(path.resolve(dest, "vendor", "src", "extensions"), plugin)
        .replace(FILE_EXT_REGEX, "")
    );

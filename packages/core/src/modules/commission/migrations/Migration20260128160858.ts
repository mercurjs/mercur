...
export class Migration20260128160858 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`alter table if exists "commission_rate" drop constraint if exists "commission_rate_code_unique";`);
    this.addSql(`create table if not exists "commission_line" ("id" text not null, "item_id" text not null, "commission_rate_id" text null, "code" text not null, "rate" real not null, "description" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "commission_line_pkey" primary key ("id"));`);

...
import { AttributeSource } from "@mercurjs/types"

import { createAndLinkAttributeValuesWorkflow } from "./create-and-link-attribute-values"

export const convertOptionToAttributeWorkflowId =
  "convert-option-to-attribute"
...
export const convertOptionToAttributeWorkflow: ReturnType<typeof createWorkflow> = createWorkflow(
  convertOptionToAttributeWorkflowId,
  (input: ConvertOptionToAttributeWorkflowInput) => {
    const createAttributeValuesInput = transform(
      { input },
      ({ input }) => ({
    ...
    const createdAttributeValues =
      createAndLinkAttributeValuesWorkflow.runAsStep({

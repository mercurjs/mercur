...
const EditCollectionMediaSchema = z.object({
  media: z.array(MediaSchema),
})

type EditCollectionMediaSchemaType = z.infer<typeof EditCollectionMediaSchema>
...
export const EditCollectionMediaForm = ({
  ...
  const handleSubmit = form.handleSubmit(async ({ media }) => {
    ...
    await mutateAsync(images, {
      onSuccess: () => {
        toast.success(t("collections.media.edit.successToast"))
        handleSuccess()
      },
      onError: (error) => {
          ...
          <div className="flex items-center justify-end gap-x-2">
            <Button variant="secondary" size="small" asChild>
              <Link to={{ pathname: ".", search: undefined }}>
                {t("collections.media.galleryLabel")}
              </Link>
            </Button>
          </div>
        </RouteFocusModal.Header>
              ...
              <div className="flex flex-col gap-y-2">
                <div className="flex flex-col gap-y-1">
                  <Text size="small" weight="plus" leading="compact">
                    {t("collections.media.label")}
                  </Text>
                  <Form.Hint>{t("collections.media.editHint")}</Form.Hint>
                </div>
                <FileUpload
            ...
            {selectionCount === 1 && (
              <Fragment>
                <CommandBar.Command
                  ...
                  label={t("collections.media.actions.setThumbnail")}
                  shortcut="t"
                />
                <CommandBar.Seperator />
                <CommandBar.Command
                  action={() => handlePromote("is_banner")}
                  label={t("collections.media.actions.setBanner")}
                  shortcut="b"
                />
                <CommandBar.Seperator />
              </Fragment>

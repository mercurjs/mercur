...
export const CollectionMediaGallery = ({
  collection,
}: CollectionMediaGalleryProps) => {
  ...
  const handleDeleteCurrent = async () => {
    ...
    const res = await prompt({
      ...
      description: t("collections.media.deleteWarning"),
    ...
    return (
      <div className="bg-ui-bg-subtle flex size-full flex-col items-center justify-center gap-y-4 pb-8 pt-6">
        ...
        <Button size="small" variant="secondary" asChild>
          <Link to="?view=edit">{t("collections.media.upload.title")}</Link>
        </Button>
      </div>
    )
  }
...
const Preview = ({
  ...
  const getVisibleItems = (items: CollectionApiImage[], index: number) => {

    ...
    for (const optionId of adminOptionIds) {
      ...
      if (sellerEntry) {
        ...
        if (input.strict) {
          throw new MedusaError(
            MedusaError.Types.INVALID_DATA,
            `Admin shipping option ${optionId} is missing seller mapping. This indicates the shipping method was not added correctly. Please remove and re-add the shipping method with the 'seller_id' parameter.`
          );
        }
      }
    }

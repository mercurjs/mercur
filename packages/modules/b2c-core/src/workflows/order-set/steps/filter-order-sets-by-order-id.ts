...
export const filterOrderSetsByOrderIdStep = createStep(
  "filter-order-sets-by-order-id",
  async (input: { orderId: string }, { container }) => {
    ...
    const orderSetId = await filterHelper.handleOrderIdFilter(input.orderId);

    return new StepResponse({
      orderSetIds: orderSetId ? [orderSetId] : [],
    });
  }
);

...
- **Active spec (2026-06-17)**: SPEC-013 (admin customers & customer groups Figma gap, MER-149/150) — `in_progress`. Landed **A0** (data model: `seller-customer-group-link.ts` flipped to one owning seller per group; link entity renamed `seller_customer_group` → `customer_group_seller`; updated the 3 pivot references in `api/vendor/customer-groups/{middlewares,helpers}.ts` + `api/vendor/customers/[id]/route.ts`) and **A** (Customer Group Owner UI: detail Owner row + list Owner column, fetched via `+seller.id,+seller.name` through Medusa's built-in admin route — no route override needed; `fields.owner` added to `en.json` + `$schema.json`). Builds green: `turbo build --filter=@mercurjs/core` and `--filter=@mercurjs/admin`. **Owed at runtime** (no DB in worktree): `medusa db:migrate` for the link-table rename (collapse duplicate group→seller rows first) + an `integration-tests/http/customer-group/admin/` test asserting the owner is returned. i18n `validate-translations.spec.ts` is pre-existing red (`sellers.fields.*`), unrelated to the `owner` key. Also landed **Gap B frontend increment** (customer-detail Orders section: added Group ID + Store columns via `order_group.display_id` / `seller.name`, reusing `orders.fields.groupId` + `fields.store`; admin build green). **Runtime-verified** (Postgres is reachable at /tmp:5432; test runner makes its own DB): running the suites caught a real bug — flipping the `defineLink` order made `remoteLink.create` 500 ("link passed in the correct order"); fixed `workflows/customer-group/steps/link-seller-customer-group.ts` to key create/dismiss `[CUSTOMER]` then `[SELLER]`. After the fix + rebuild: `customer-group/vendor` 14/14, `customer/vendor` 8/8, and a NEW `customer-group/admin/customer-group.spec.ts` (3/3) proving `GET /admin/customer-groups[/:id]?fields=+seller.id,+seller.name` returns the owner (and none for platform groups) — so Gap A needs no admin route override. **Owed**: production data migration for the link-table rename (`seller_customer_group` → `customer_group_seller` is a different table; copy/collapse owner rows before dropping old; fresh DBs incl. tests unaffected). **Gap B**: frontend columns landed + admin build green, but order-LIST resolution of `order_group`/`seller` not yet runtime-verified (heavy multi-vendor-order setup); full expandable tree still pending design/PM. **Not started**: Gap C (alignment, needs running app). See `docs/specs/SPEC-013-*.md` Evidence.
